...
| 我的 | 学习统计、**每组词量可自定义（1–200）**、拓展页开关、云端上传 / 拉取、重置进度 |
...
`images/theme/01.jpg` ~ `22.jpg` 对应 22 个章节，由 AI 生成后统一裁切压缩（720×405、JPEG q74、合计约 231 KB）。
生成时的统一约束是：扁平极简、柔和低饱和配色、大量留白、无文字、16:9 横版。
换图时只要保持文件名与章节顺序一致即可，`utils/word.js` 的 `themeImage()` 负责映射。
