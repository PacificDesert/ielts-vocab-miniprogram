...
一个 iOS 简约风格的雅思背单词小程序：词汇浏览记忆、拼写自测、**深浅色主题**、换词翻书动画，保留云开发后端做数据持久化，天然支持多人使用。
...
| 模块 | 说明 |
| --- | --- |
| 学习（记忆卡） | 卡片式记忆，轻触翻面看释义与例句，「认识 / 不认识」驱动熟练度分级，换卡带翻书动画 |
| 单词拓展 | 标记「认识」后进入：章节配图 + 书中例句 + 词形拓展 + 相近单词；每个词形 / 相近词都有独立发音按钮，点相近词可继续联想，底部「下一个单词」会**跳回记忆单词的大卡片接着背** |
| 词库 | 22 章节分组浏览 + 中英文搜索 |
| 拼写自测 | 看中文释义拼写英文，支持逐字母提示、跳过，统计正确率 |
| 意思匹配 | 一组背完后进入：左列英文、右列中文各打乱，先点英文再点中文配对；每屏 4 对，配错会抖动且可重选，做完自动翻下一屏并统计正确率 |
| 单词详情 | 大字单词、音标、释义、**书中例句**、词形拓展、在线发音，上一个 / 下一个翻书式切换 |
| 主题 | 深色 / 浅色 / 跟随系统三种模式，导航栏、tabBar、页面配色整体联动 |
| 我的 | 学习统计、**外观切换**、**每组词量手动输入（下限 1，不设上限）**、拓展页开关、**多设备自动同步**、隐私政策、重置进度 |
| 后端 | 微信云开发云函数 `login` / `data`，按 openid 一人一条记录；**按单词逐条合并，两台设备各背一半不会互相覆盖** |
...
```
├── app.js / app.json / app.wxss     全局逻辑、深浅色设计变量与翻书动画
├── theme.json                       darkmode 主题变量（导航栏 / tabBar 跟随系统）
├── config.js                        云环境 ID、发音接口、计划默认值与上下限
├── data/words.js                    词库（由 PDF 生成，勿手改）
├── utils/
│   ├── cloud.js                     云函数调用封装（未配置时自动降级为本地模式）
│   ├── store.js                     学习进度：本地存储 + 云端同步
│   ├── theme.js                     浅色 / 深色 / 跟随系统三种模式与原生控件覆盖
│   ├── flip.js                      翻书动画助手（换内容时先翻走再翻回来）
│   └── word.js                      词库检索、章节、配图、相近词、词卡配色
├── pages/
│   ├── index/                       学习首页
│   ├── vocab/                       词库浏览与搜索
│   ├── detail/                      单词详情
...
它用真实词库渲染，搜索、翻卡、拼写作答、翻书动画、深浅色切换都能实际操作，进度存在浏览器本地。
...
配置云环境后是**自动双向同步**，不需要手动点上传下载：

- **启动时**：拉取云端进度，与本地逐条合并后写回，并刷新当前页面
- **学习中**：标记单词或改设置后延迟 2 秒推送一次（防抖，避免连点反复调云函数）
- **切回前台**：距上次同步超过 60 秒会再同步一次
- **冲突合并**：按单词逐条比较修改时间取较新一方；每日计数取较大值；学习游标取较大值。
  因此「手机背了 A 部分、平板背了 B 部分」合并后两份都在，不会互相覆盖
- 「我的」页面可看到上次同步时间、手动「立即同步」，也可以关掉自动同步

未配置 `CLOUD_ENV` 时以上全部跳过，程序走纯本地模式。

## 上架

小程序上架需要在微信公众平台完成注册、ICP 备案、服务类目选择、用户隐私保护指引填写、
服务器合法域名配置，最后提交审核发布。相关信息请以微信官方文档与小程序后台的实时提示为准。

⚠️ **词库版权风险**：词库与例句整理自公开出版物，公开发布属于再分发，
如需正式对外发布，请先替换为无版权限制的词库，或仅作个人学习使用。
...
本工程在此架构上重写为 iOS 简约风格界面，替换词库，并加入拼写自测、深浅色主题、翻书动画与单词拓展页。
